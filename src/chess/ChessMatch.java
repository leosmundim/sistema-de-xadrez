...
public class ChessMatch {
	...
	public ChessPiece performChessMove(ChessPosition sourcePosition, ChessPosition targetPosition) {
		...
		//#Movimento especial En Passant do peão
		//Se a peca que se moveu é um peao e se ele se movimentou duas casas (branco ou preto) ele esta sujeito ao En Passant
	...
	public ChessPiece replacePromotedPiece (String type) {
		...
		if (!type.equals("B") && !type.equals("C") && !type.equals("T") && !type.equals("Q")) {
			throw new ChessException("Peca invalida para promocao!");
		}
	...
	private Piece makeMove(Position source, Position target) {
		...
		board.placePiece(p, target); // placePiece recebe um Piece, o upCasting é feito automaticamente pois P é uma ChessPiece
		...
		if (p instanceof Pawn) {
			//se meu peao andou na diagonal (mudou de coluna) e não capturou peca, foi um en passant
			if (source.getColumn() != target.getColumn() && capturedPiece == null) {
				Position pawnPosition;
				if (p.getColor() == Color.WHITE) {
					// se for uma peca branca, o peao a ser capturado esta embaixo dele
					pawnPosition = new Position (target.getRow() + 1, target.getColumn());
				}
				else {
					pawnPosition = new Position (target.getRow() - 1, target.getColumn());
				}
				//removendo a peca capturada das pecas do tabuleiro e adicionando na lista de pecas capturadas
	...
	private void undoMove(Position source, Position target, Piece capturedPiece) {
		...
		//#Movimento especial En Passant
		if (p instanceof Pawn) {
			//se meu peao andou na diagonal (mudou de coluna) e a peca capturada foi um peao vulneravel  entao foi um en passant
			if (source.getColumn() != target.getColumn() && capturedPiece == enPassantVulnerable) {
				...
				//colocar a peca pawn na posicao correta
				board.placePiece(pawn, pawnPosition);
				
				//nao preciso fazer a troca de lista pq o algoritmo ja fez no codigo la em cima
			}
		}
	}
	...
	private boolean testCheckMate(Color color) {
		...
		for (Piece p : list) {
			...
			for(int i=0; i<board.getRows(); i++) {
				for (int j=0; j<board.getColumns(); j++) {
					//para cada elemento da matriz, tenho que testar se o movimento é possível:
					if (mat[i][j]) {
	...
	private void initialSetup() {
        ...
        placeNewPiece('e', 8, new King(board, Color.BLACK, this)); //#this referente a chessMatch onde estou conforme construtor
